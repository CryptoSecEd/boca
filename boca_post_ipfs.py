...
and uploads it to IPFS. The file can optionally be encrypted as well,
but it is still the plaintext file hash that gets posted to the
blockchain.
...
from boca.ipfs import upload_to_ipfs, zipdir, hash_file


def main():
    """Post a hash of a document to a blockchain and upload the file to
    the IPFS network."""

    parser = ArgumentParser(description="Post to a blockchain using the " +
    ...
    parser.add_argument("--addcid", help="As well as posting the file hash " +
                        "to the blockchain, this also adds the ipfs CID",
                        action="store_true")
    ...
    if args.addcid:
        if (not args.enc and not args.selfenc):
            # I could enable args.enc, but I don't want encryption to
            # happen without the user realising it.
            print("Warning! The '--addcid' option without encryption means " +
                  "your data will be publicly available. You are recommended" +
                  " to use '--enc' to encrypt your data.")
            user_choice = input("If you still wish to proceed without " +
                                "encryption, please press Y/y. " +
                                "Otherwise. Press any other key to exit: "
                                )
            if user_choice not in ('Y', 'y'):
                print("Exiting")
                sys.exit(0)
        print("Warning! This option makes your data publicly available. " +
              "Please use with care.")

    if not args.path:
        print("You must specify a file (or folder) to be posted.")
        print("Use the --path argument.")
        sys.exit(1)
    else:
    ...
    if input_path.is_dir():
        # Need to create an index file, post the directory to IPFS and
        # post the hash of the index file to the Blockchain
        index_path = Path(input_path, 'index.txt')
        # Give the user to delete the existing index.txt file if it is
        # already there. If you use an old index.txt file and add files
        # to the folder, then they won't be hashed and put on the
        # blockchain.
    ...
    if args.enc:
        ...
        if existing_filename.is_file():
            ...
            if response in ('Y', 'y'):
                ...
                enc_filename = existing_filename
            else:
                print("Invalid input")
                sys.exit(1)
        ...
        res = upload_to_ipfs(enc_filename)
        print("File uploaded to ipfs: %s" % res['Hash'])
        if args.addcid:
            string_to_post = string_to_post + " " + res['Hash']
    else:
        res = upload_to_ipfs(input_path)
        print("Uploaded '%s' to IPFS" % input_path)
        if input_path.is_dir():
            for ipfs_detail in res:
                if ipfs_detail['Name'] == str(input_path):
                    print("Directory uploaded to ipfs: %s" %
                          ipfs_detail['Hash'])
                    if args.addcid:
                        string_to_post = (string_to_post + " " +
                                          ipfs_detail['Hash'])
        else:
            print("File uploaded to ipfs: %s" % res['Hash'])
            if args.addcid:
